...
static void SwitchToUpdates(void* w)       { /* updates removed */ }
...
static struct DirectConnectScreen {
	...
	struct LInput iptUsername, iptAddress;
...
static void SettingsScreen_DPIScaling(struct LCheckbox* w) {
	...
	Window_ShowDialog("Restart required", "You must restart VoxelCraft before display scaling takes effect");
#else
